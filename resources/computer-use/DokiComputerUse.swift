...
// input delivery, sessions, and native authorization. Doki owns TCC onboarding
                ...
                do {
                    ...
                    case "initialize":
                        result = ["protocolVersion": "2025-06-18", "capabilities": ["tools": [:]],
                                  "serverInfo": ["name": "Doki Cua Driver", "version": "0.28.0"]]
...
private func readExactly(_ count: Int, from input: FileHandle) throws -> Data? {
    ...
    while data.count < count {
        ...
        if chunk.isEmpty {
            ...
            throw CuaError("the Doki connection closed mid-message")
        }
        data.append(chunk)
    }
    return data
...
private func connectedChannel(at path: String) throws -> (FileHandle, pid_t) {
    ...
    do {
        ...
        guard getsockopt(descriptor, SOL_LOCAL, LOCAL_PEERPID, &peerPID, &peerPIDSize) == 0 else {
            throw CuaError("could not identify the Doki process")
        }
